...
def find_match(cids, rgbs, rgb_name, taken, cnt=8):
...
#cid_name_rgb = load_matching("matching1")
#print("matching1", len(cid_name_rgb))
cid_name_rgb = load_matching("matching2")
print("matching2", len(cid_name_rgb))
...
for i, (name, cids) in enumerate(ditig_name_cid.items()):
  ...
  taken = set(k for k in ditig_name_cid.keys() if k != name)

  picks = find_match(cids, rgbs, rgb_name, taken)
  if picks is None:
    picks = find_match(cids, rgbs, rgb_name, taken, cnt=20)
